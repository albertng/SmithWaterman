...
int main(int argc, char* argv[])
{
    ...
    // Scoring parameters: {SUB_AA, SUB_AC, SUB_AG, SUB_AT, SUB_CC, SUB_CG, SUB_CT, SUB_GG, SUB_GT, SUB_TT, GAP_OPEN, GAP_EXTEND}
    int32_t score_param_buf[] = {2, -2, -2, -2, 2, -2, -2, 2, -2, 2, -2, -1};
    ...
    for (int i = 0; i < num_fpgas; i++) {   
        if ((err = pico[i]->WriteDeviceAbsolute(0, (uint32_t*) score_param_buf, 48)) < 0) {
            fprintf(stderr, "Error Writing Scoring Parameters for FPGA %d: %d\n", i, err);
            exit(1);
        }
    }
