...
localparam verbose = 0;
    ...
    reg [31:0]      irem_seq_table[511:0], irem_seq_p5, irem_seq_p4, irem_seq_p3, irem_seq_pp, irem_seq_pre, irem_seq, irem_seq_table_wr_data;
    reg [8:0]       irem_seq_table_wr_inx;
    reg             irem_seq_table_wr_en, rem_seq_wr_en;
    ...
    reg [8:0]       istream_table_inx=9'h0, istream_desc_inx_p7, istream_desc_inx_p6, istream_desc_inx_p5, istream_desc_inx_p4, istream_desc_inx_p3, istream_desc_inx_pp, isd_inx_p, isd_inx, isd_wr_inx;
    ...
    always @(posedge clk) begin
        ...
        istream_desc_inx_p7 <= istream_inx_map_table[(istream_table_inx + 1) & istream_inx_mask]; // clumsy, and potential timing problem ...
        ...
        if (rst_q) begin
            // on startup, we actually go into the ISTREAM_STATE_CLEAR state before seeing reset.
            // thus it's critical to clear the rd_tag_fifo enable when we go into reset so that we don't
            //   write the first tag (0) twice.
            rd_tag_fifo_init_en     <= 0;
            // make sure we don't write to the fifo when we temporarily jump into the "clear" state prior to getting the reset.
            rd_tag_fifo_rst_done    <= 1;
            // also, when we finally hit the ISTREAM_STATE_CLEAR state "for real," istream_table_inx will start at 2, rather than 0.
            // that means we'll miss adding the tags "0" and "1", but that's probably better than stressing the istream_table_inx with
            //   a reset, which is the only straightforward alternative.
            // this line makes us able to handle repeated resets. for example, if we come up at power-on, and then the bios POST does a reset.
            //   (this is why we used to have problems with putting 5.x firmware on the cards, and they required a reload before they were sane.)
          ...
          case (istream_state)
            ISTREAM_STATE_CLEAR : begin
                ...
                if (stream_machine_en && (istream_table_inx == 9'h1ff))
            ...
            ISTREAM_STATE_RUN : begin
                ...
                if (rem_seq_wr_en) begin
                    ...
                    irem_seq_table_wr_en    <= 1;
                    irem_seq_table_wr_inx   <= wr_addr[8+2:0+2];
                    irem_seq_table_wr_data  <= wr_data[31:0];
                end
